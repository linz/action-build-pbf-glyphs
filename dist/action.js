  ...
  async *list(filePath, opts) {
    try {
      ...
      for (const file of files) {
        ...
        if (file.isDirectory() && (opts === null || opts === void 0 ? void 0 : opts.recursive) !== false)
  ...
  async head(filePath) {
    try {
      ...
      if (isRecord(e2) && e2.code === "ENOENT")
        return null;
      throw getCompositeError(e2, `Failed to stat: ${filePath}`);
    }
  }
  async read(filePath) {
    try {
...
var FsHttp = class {
  ...
  async head(filePath) {
    ...
    if (!res.ok)
      throw getCompositeError2(new Error(res.statusText), `Failed to head: ${filePath}`);
...
var FileSystemAbstraction = class {
  constructor() {
    ...
    this.join = joinUri;
    this.joinAll = joinAllUri;
  }
  register(path4, system, flag = "rw") {
    for (let i2 = 0; i2 < this.systems.length; i2++) {
      const sys = this.systems[i2];
      if (sys.path === path4 && sys.flag === flag) {
        this.systems.splice(i2, 1, { path: path4, system, flag });
        return;
      }
    }
    this.systems.push({ path: path4, system, flag });
  ...
  details(filePath, opts) {
    return this.get(filePath, "r").details(filePath, opts);
  }
  exists(filePath) {
    return this.get(filePath, "r").head(filePath).then((f3) => f3 != null);
  }
  head(filePath) {
...
async function findGlyphCommand() {
  const glyphCommand = "./static/build_pbf_glyphs";
  const isInstalled = await fileExists(glyphCommand);
  if (isInstalled)
    return glyphCommand;
  const upCommand = import_path3.default.join(__dirname, "..", glyphCommand);
  const isInstalledUp = await fileExists(upCommand);
  if (isInstalledUp)
    return upCommand;
  throw new Error("Failed to open command @ " + glyphCommand);
}
async function waitForChildProcess(cmd, args) {
  core.info(`	 ${cmd} ${args}`);
  const child = import_child_process.default.spawn(cmd, args);
  const stdout = [];
  const stderr = [];
  if (child.stdout == null)
    throw new Error("Child missing stdout");
  if (child.stderr == null)
    throw new Error("Child missing stderr");
...
// src/action.ts
function isFont(f3) {
  return f3.toLowerCase().endsWith(".ttf") || f3.toLowerCase().endsWith(".otf");
}
async function main() {
  const SourceLocation = core2.getInput("source");
  ...
  for (const folder of fontFolders) {
    core2.info(`Creating Glyphs: ${folder}`);
